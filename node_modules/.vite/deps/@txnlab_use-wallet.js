...
// node_modules/@txnlab/use-wallet/dist/esm/index.js
...
var getAlgosdk = async () => {
  return (await import("./esm-EA2W2NJ7.js")).default;
};
...
var Algod = class _Algod {
  constructor(algosdk, algodClient) {
    __publicField(this, "algosdk");
  ...
  static async init(algodOptions) {
    ...
    return new _Algod(algosdk, algodClient);
  }
};
...
var getIsIOS = () => {
  if (typeof window !== "undefined") {
    return /iPad|iPhone|iPod/.test(navigator == null ? void 0 : navigator.userAgent) && !(window == null ? void 0 : window.MSStream);
  } else {
    return false;
  }
};
...
var BaseClient = class {
  constructor(metadata, algosdk, algodClient, clientOptions) {
    ...
    __publicField(this, "keepWCAlive");
    __publicField(this, "metadata");
    __publicField(this, "decodeTransaction", (txn, isSigned) => {
...
function requireWithSelector_development() {
  ...
  if (true) {
    (function() {
      ...
      var useRef = React.useRef, useEffect2 = React.useEffect, useMemo2 = React.useMemo, useDebugValue2 = React.useDebugValue;
...
var useWalletStore = create()(immer(persist(devtools((set, _get) => ({
  ...
  clearActiveAccount: (id) => {
    set((state) => {
      var _a;
      if (id === ((_a = state.activeAccount) == null ? void 0 : _a.providerId))
...
var useHydratedWalletStore = (selector, compare) => {
...
var _client;
var _PeraWalletClient = class _PeraWalletClient extends BaseClient {
  ...
  async connect(onDisconnect) {
    ...
    (_a = __privateGet(this, _client).connector) == null ? void 0 : _a.on("disconnect", onDisconnect);
...
_client = new WeakMap();
__publicField(_PeraWalletClient, "metadata", {
  id: PROVIDER_ID.PERA,
  name: "Pera",
  icon: ICON$8,
...
var _DaffiWalletClient = class _DaffiWalletClient extends BaseClient {
  ...
  async connect(onDisconnect) {
    ...
    (_a = __privateGet(this, _client2).connector) == null ? void 0 : _a.on("disconnect", onDisconnect);
    if (accounts.length === 0) {
...
__publicField(_DaffiWalletClient, "metadata", {
...
var _client3;
var _MyAlgoWalletClient = class _MyAlgoWalletClient extends BaseClient {
  constructor({ metadata, client, clientOptions, algosdk, algodClient, network }) {
    super(metadata, algosdk, algodClient);
  ...
  async signTransactions(connectedAccounts, transactions, indexesToSign, returnGroup = true) {
    ...
    const txnsToSign = decodedTxns.reduce((acc, txn, i2) => {
      const isSigned = "txn" in txn;
      if (indexesToSign && indexesToSign.length && (indexesToSign == null ? void 0 : indexesToSign.includes(i2))) {
        signedIndexes.push(i2);
        acc.push(transactions[i2]);
      } else if (!isSigned && connectedAccounts.includes(this.algosdk.encodeAddress(txn["snd"]))) {
        signedIndexes.push(i2);
    ...
    const signedTxns = transactions.reduce((acc, txn, i2) => {
      var _a;
      if (signedIndexes.includes(i2)) {
        const signedByUser = (_a = result.shift()) == null ? void 0 : _a.blob;
        signedByUser && acc.push(signedByUser);
      } else if (returnGroup) {
        acc.push(txn);
      }
...
__publicField(_MyAlgoWalletClient, "metadata", {
...
var _client4;
var _DeflyWalletClient = class _DeflyWalletClient extends BaseClient {
  constructor({ metadata, client, clientOptions, algosdk, algodClient, network }) {
    super(metadata, algosdk, algodClient);
  ...
  async connect(onDisconnect) {
    ...
    (_a = __privateGet(this, _client4).connector) == null ? void 0 : _a.on("disconnect", onDisconnect);
...
__publicField(_DeflyWalletClient, "metadata", {
...
var _client5;
var _ExodusClient = class _ExodusClient extends BaseClient {
  constructor({ metadata, client, clientOptions, algosdk, algodClient, network }) {
    super(metadata, algosdk, algodClient);
...
__publicField(_ExodusClient, "metadata", {
...
var _client6;
var _AlgoSignerClient = class _AlgoSignerClient extends BaseClient {
  ...
  getAuthAddress(address) {
    ...
    return account == null ? void 0 : account.authAddr;
  }
};
_client6 = new WeakMap();
__publicField(_AlgoSignerClient, "metadata", {
...
var _client7, _composeTransactions, composeTransactions_fn, _getSession, getSession_fn, _mapAccounts, mapAccounts_fn;
var _WalletConnectClient = class _WalletConnectClient extends BaseClient {
  ...
  async connect() {
    ...
    try {
      const session = await __privateGet(this, _client7).connect({
        requiredNamespaces
      });
  ...
  async reconnect() {
    const session = await __privateGet(this, _client7).getSession();
  ...
  async disconnect() {
    try {
      ...
      await __privateGet(this, _client7).disconnect({
        topic: session.topic,
  ...
  async signTransactions(connectedAccounts, txnGroups, indexesToSign = [], returnGroup = true) {
    ...
    const session = await __privateMethod(this, _getSession, getSession_fn).call(this);
    const response = await __privateGet(this, _client7).request({
      chainId: this.chain,
      topic: session.topic,
      request
    });
...
__publicField(_WalletConnectClient, "metadata", {
...
var _KMDWalletClient = class _KMDWalletClient extends BaseClient {
  ...
  async getWalletToken(walletId, password) {
    const handleResp = await __privateGet(this, _client8).initWalletHandle(walletId, password);
    return handleResp.wallet_handle_token;
  }
  async releaseToken(token) {
    await __privateGet(this, _client8).releaseWalletHandle(token);
  }
  async listWallets() {
    const walletResponse = await __privateGet(this, _client8).listWallets();
  ...
  async listAccounts(wallet, password) {
    ...
    const listResponse = await __privateGet(this, _client8).listKeys(token);
  ...
  async signTransactions(connectedAccounts, txnGroups, indexesToSign, returnGroup = true) {
    ...
    decodedTxns.forEach((dtxn, idx) => {
      ...
      signingPromises.push(__privateGet(this, _client8).signTransaction(token, pw, txn));
    });
...
__publicField(_KMDWalletClient, "metadata", {
  id: PROVIDER_ID.KMD,
  name: "KMD",
  icon: ICON$1,
  isWalletConnect: false
...
var ICON = "data:image/svg+xml,%3c%3fxml version='1.0' encoding='UTF-8'%3f%3e %3c!-- Generated by Pixelmator Pro 3.2.2 --%3e %3csvg width='409' height='210' viewBox='0 0 409 210' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'%3e%3ctext id='MNEMONIC' xml:space='preserve' x='0' y='129' font-family='Helvetica' font-size='72' fill='black'%3eMNEMONIC%3c/text%3e%3c/svg%3e";
var _client9;
var _MnemonicWalletClient = class _MnemonicWalletClient extends BaseClient {
  ...
  async connect() {
    ...
    if (!password) {
      __privateSet(this, _client9, void 0);
      throw new Error("Mnemonic passphrase is required");
    }
    __privateSet(this, _client9, this.algosdk.mnemonicToSecretKey(password));
    return {
      ..._MnemonicWalletClient.metadata,
      accounts: [
        {
          name: `MnemonicWallet 1`,
          address: __privateGet(this, _client9).addr,
  ...
  signTransactions(connectedAccounts, txnGroups, indexesToSign, returnGroup = true) {
    ...
    decodedTxns.forEach((dtxn, idx) => {
      ...
      signedTxns[idx] = new Uint8Array();
      const txn = this.algosdk.Transaction.from_obj_for_encoding(dtxn);
      const signedTxn = txn.signTxn((_a = __privateGet(this, _client9)) == null ? void 0 : _a.sk);
...
__publicField(_MnemonicWalletClient, "metadata", {
...
var reconnectProviders = async (providers) => {
  try {
    ...
    for (const client of clients) {
      const id = client == null ? void 0 : client.metadata.id;
...
function useWallet() {
  ...
  const clients = (0, import_react.useContext)(ClientContext);
  ...
  const connectedActiveAccounts = (0, import_react.useMemo)(() => connectedAccounts.filter((account) => account.providerId === (activeAccount == null ? void 0 : activeAccount.providerId)), [connectedAccounts, activeAccount]);
  (0, import_react.useEffect)(() => {
    ...
    setProviders(supportedClients.map((id) => {
      return {
        ...
        isActive: (activeAccount == null ? void 0 : activeAccount.providerId) === id,
  ...
  const getClient = (id) => {
    ...
    const walletClient = clients == null ? void 0 : clients[id];
    if (!walletClient)
  ...
  const isReady = (0, import_react.useMemo)(() => {
  ...
  const connect = async (id) => {
    try {
      ...
      const walletInfo = await (walletClient == null ? void 0 : walletClient.connect(() => clearAccounts(id)));
  ...
  const reconnect = async (id) => {
    try {
      const walletClient = getClient(id);
      const walletInfo = await (walletClient == null ? void 0 : walletClient.reconnect(() => clearAccounts(id)));
      if (walletInfo && walletInfo.accounts.length) {
        addAccounts(walletInfo.accounts);
      }
    } catch (e) {
  ...
  const disconnect = async (id) => {
    try {
      const walletClient = getClient(id);
      await (walletClient == null ? void 0 : walletClient.disconnect());
    } catch (e) {
      console.error(e);
    } finally {
      clearAccounts(id);
  ...
  const signTransactions = async (transactions, indexesToSign, returnGroup = true) => {
    const walletClient = getClient(activeAccount == null ? void 0 : activeAccount.providerId);
    if (!walletClient || !(activeAccount == null ? void 0 : activeAccount.address)) {
  ...
  const sendTransactions = async (transactions, waitRoundsToConfirm) => {
    const walletClient = getClient(activeAccount == null ? void 0 : activeAccount.providerId);
    const result = await (walletClient == null ? void 0 : walletClient.sendRawTransactions(transactions, waitRoundsToConfirm));
    return result;
  };
  ...
  const getAccountInfo = async () => {
    ...
    const accountInfo = await (walletClient == null ? void 0 : walletClient.getAccountInfo(activeAccount.address));
    return accountInfo;
  };
  const getAddress = () => {
    return activeAccount == null ? void 0 : activeAccount.address;
  };
  const getAssets = async () => {
    if (!activeAccount)
      throw new Error("No selected account.");
    const walletClient = getClient(activeAccount.providerId);
    return await (walletClient == null ? void 0 : walletClient.getAssets(activeAccount.address));
  };
  const groupTransactionsBySender = (transactions) => {
    const walletClient = getClient(activeAccount == null ? void 0 : activeAccount.providerId);
    return walletClient == null ? void 0 : walletClient.groupTransactionsBySender(transactions);
  };
  return {
    clients,
    providers,
    ...
    activeAccount,
    activeAddress: activeAccount == null ? void 0 : activeAccount.address,
    status,
    isActive,
    isReady,
    signer,
...
function useInitializeProviders({ providers, nodeConfig, algosdkStatic, debug = false }) {
  ...
  (0, import_react.useEffect)(() => setDebug(debug), [debug, setDebug]);
  const [walletProviders, setWalletProviders] = (0, import_react.useState)(null);
  (0, import_react.useEffect)(() => {
...
export {
  ...
  DEFAULT_NODE_TOKEN,
  PROVIDER_ID,
  clientStore as WalletProvider,
  AlgoSignerClient as algosigner,
  DeflyWalletClient as defly,
  encodeNFDTransactionsArray,
  ExodusClient as exodus,
  KMDWalletClient as kmd,
  MnemonicWalletClient as mnemonic,
  MyAlgoWalletClient as myalgo,
  PeraWalletClient as pera,
  reconnectProviders,
  useInitializeProviders,
  useWallet,
  WalletConnectClient as walletconnect
};
...
@txnlab/use-wallet/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=@txnlab_use-wallet.js.map
